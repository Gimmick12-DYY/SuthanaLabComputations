...
def bayesian_group_inference(data, param_name, condition):
    ...
    plt.suptitle(f'Posterior of {param_name} - {condition} condition (24h Window)')
    
    # Save the plot with descriptive filename
    param_clean = param_name.replace('(', '').replace(')', '').replace('mean', '').strip()
    filename = f"posterior_{param_clean}_{condition.lower()}_24h_window.png"
...
def compare_conditions(pre_data, post_data, param_name):
    ...
    ax1.set_title(f'{param_name} - Pre vs Post Comparison (24h Window)')
    ...
    ax2.set_title(f'{param_name} - Distribution Comparison (24h Window)')
    ...
    param_clean = param_name.replace('(', '').replace(')', '').replace('mean', '').strip()
    filename = f"comparison_{param_clean}_pre_vs_post_24h_window.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()

# --- Run Bayesian Analysis for Each Condition ---
print("\n" + "="*50)
print("BAYESIAN ANALYSIS - PRE CONDITION (24h Window)")
print("="*50)
...
print("\n" + "="*50)
print("BAYESIAN ANALYSIS - POST CONDITION (24h Window)")
print("="*50)
...
# --- Comparison Plots ---
print("\n" + "="*50)
print("COMPARISON PLOTS (24h Window)")
print("="*50)
...
print("\n" + "="*50)
print("SUMMARY STATISTICS FOR COMPARISON (24h Window)")
print("="*50)
