"""
This is the adjusted CosinorPy module for TRPTSD analysis.
It includes functions for periodogram analysis, linear component removal, model fitting, and plotting.
Changes include:
- Improved handling of dataframes for periodogram and linear component removal.
- Enhanced plotting functions for better visualization of results.
- Adjusted functions for better compatibility with TRPTSD data.
"""
...
def pivot_data(df, merge_repeats = True, z_score = True, df_results=False, sort_by="p", ascending=True, filter_significant=False, dropnacols=False):
    ...
    df_heatmap = df_heatmap.pivot(index="test", columns="x", values="y")
...
def calculate_statistics(X, Y, Y_fit, n_components, period, lin_comp = False):
    ...
    for x in X_unique:
        ...
        print(Y[X_periodic == x], ', ', Y_i_avg)
        SSPE += sum((Y[X_periodic == x] - Y_i_avg)**2)
    SSLOF = RSS-SSPE
    print('n_T: ', n_T)
    print('X_preiodic: ', X_periodic)
    print('RSS: ', RSS)
    print('SSPE: ', SSPE)
    print('SSLOF: ', SSLOF)
    if lin_comp:
